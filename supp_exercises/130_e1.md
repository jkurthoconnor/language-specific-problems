...
def missing(array)
  ...
  missing = range.delete_if { |n| array.include?(n) }
  missing
end
```
...
or
```ruby
def rot13(encrypted_array)
  decrypted_array = []
  encrypted_array.each do |name|
    n = ''
    name.each_char do |ltr|
      if (65..77).include?(ltr.ord) || (97..109).include?(ltr.ord)
        n << (ltr.ord + 13).chr
      elsif (78..90).include?(ltr.ord) || (110..122).include?(ltr.ord)
        n << (ltr.ord - 13).chr
      else
        n << ltr
      end
    end
    decrypted_array.push(n)
  end
  decrypted_array
end
```
...
#### Solution:
No, my solution only works for arrays.  Because I looped manually through the collection using index incrementation, my method would need to be adapted to work with a non-indexed collection.
