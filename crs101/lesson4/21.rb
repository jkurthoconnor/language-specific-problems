...
DECK.each_pair { |k, v| puts "This is the key: #{k} \n\tand this is the value #{v}" }
 ...
 deck_in_play = DECK.select { |k, v| k == k}.to_a
 # dealing
 deck_in_play.shuffle # must delete sample OR could use deck_in_play.shuffle and a series of .shift or .pop calls

 hands = { dealer: [], player: []

 # dealing cards via keys.sample will result in only dealing keys.
 ## must then use DECK as reference to retrieve value
 ## OR store DECK and/or deck_in_play as an array of arrays, [ ['King of Diamonds', 10], ['Ace of Diamonds', [1, 11]] ]
