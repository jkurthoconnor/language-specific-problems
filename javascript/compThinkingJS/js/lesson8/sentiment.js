...
var positiveRegex = /(fortunes?)|(dream(s|t|ed)?)|(love(s|d)?)|(respect(s|ed)?)|(patien(ce|t)?)|(devout(ly)?)|(nobler?)|(resolut(e|ion)?)/g;

var negativeRegex = /(die(s|d)?)|(heartached?)|(death)|(despise(s|d)?)|(scorn(s|ed)?)|(weary)|(troubles?)|(oppress(es|ed|or('s)?)?)/g;

function sentiment(text) {
  var sentiment;
  var words = text.toLowerCase().replace(/[\[\],.;:?!\-]/g, ' ').split(/\s+/);
  var [positives, negatives] = idPositiveNegative(words);
...
function idPositiveNegative(words) {
  var positive = [];
  var negative = [];
